...
// MARK: - 路径/URL
public extension SaberEx where Base: FileManager {
    /// `iCloud`目录的完整路径
    /// - Returns: `String`
...
public extension SaberEx where Base: FileManager {
    ...
    static func isExists(_ path: String) -> Bool {
        let exist = Base.default.fileExists(atPath: path)
        guard exist else {
            return false
        }
        return true
    ...
    static func previousPath(_ fullPath: String) -> String {
        return fullPath.sb.toNSString().deletingLastPathComponent
    }
    ...
    static func isReadable(_ path: String) -> Bool {
        return Base.default.isReadableFile(atPath: path)
    }
    ...
    static func isWritable(_ path: String) -> Bool {
        return Base.default.isWritableFile(atPath: path)
    }
    ...
    static func isExecutable(_ path: String) -> Bool {
        return Base.default.isExecutableFile(atPath: path)
    }
    ...
    static func isDeletable(_ path: String) -> Bool {
        return Base.default.isDeletableFile(atPath: path)
    }
    ...
    static func pathExtension(_ path: String) -> String {
        return path.sb.toNSString().pathExtension
    }
    ...
    static func shallowSearchAllFiles(_ path: String) -> [String] {
        guard let result = try? Base.default.contentsOfDirectory(atPath: path) else {
            return []
        }
        return result
    }
    ...
    static func allFiles(_ path: String) -> [String] {
        guard
            isExists(path),
            let subPaths = Base.default.subpaths(atPath: path)
        else {
            return []
        }
        return subPaths
    ...
    static func deepSearchAllFiles(_ path: String) -> [Any]? {
        guard isExists(path),
              let contents = Base.default.enumerator(atPath: path)
        else {
            return nil
        }
    ...
    static func attributeList(_ path: String) -> [FileAttributeKey: Any]? {
        do {
            return try Base.default.attributesOfItem(atPath: path)
        } catch {
            return nil
        }
    }
    ...
    static func fileSize(_ path: String) -> UInt64 {
        ...
        guard let attributes = try? Base.default.attributesOfItem(atPath: path),
    ...
    static func folderSize(_ path: String) -> String {
        if path.count == 0, !Base.default.fileExists(atPath: path) {
            return "0KB"
        }

        var fileSize: UInt64 = 0
        do {
            let files = try Base.default.contentsOfDirectory(atPath: path)
            for file in files {
                let path = path + "/\(file)"
    ...
    static func isEqual(path1: String, path2: String) -> Bool {
        ...
        return Base.default.contentsEqual(atPath: path1, andPath: path2)
    }
    ...
    static func createFolder(_ path: String) -> (isSuccess: Bool, error: String) {
        ...
        do {
            try Base.default.createDirectory(atPath: path, withIntermediateDirectories: true, attributes: nil)
            return (true, "创建成功!")
        } catch {
            return (false, "创建失败")
        }
    ...
    static func removeFolder(_ path: String) -> (isSuccess: Bool, error: String) {
        ...
        do {
            try Base.default.removeItem(atPath: path)
            return (true, "删除成功!")
        } catch _ {
            return (false, "删除失败!")
        }
    ...
    static func createFile(_ path: String) -> (isSuccess: Bool, error: String) {
        guard isExists(path) else {
            let ok = Base.default.createFile(atPath: path, contents: nil, attributes: nil)
    ...
    static func readFile(_ path: String) -> String? {
        ...
        let data = Base.default.contents(atPath: path)
    ...
    static func copyItem(from fromPath: String, to toPath: String, isFile: Bool = true, isCover: Bool = true) -> (isSuccess: Bool, error: String) {
        ...
        if isCover, isExists(toPath) {
            do {
                try Base.default.removeItem(atPath: toPath)
            } catch {
                return (false, "拷贝失败!")
            }
        }
        ...
        do {
            try Base.default.copyItem(atPath: fromPath, toPath: toPath)
        } catch {
            return (false, "拷贝失败!")
        }
        return (true, "拷贝成功!")
    ...
    static func moveItem(from fromPath: String, to toPath: String, isFile: Bool = true, isCover: Bool = true) -> (isSuccess: Bool, error: String) {
        ...
        do {
            try Base.default.moveItem(atPath: fromPath, toPath: toPath)
        } catch {
            return (false, "移动失败!")
        }
        return (true, "移动成功!")
