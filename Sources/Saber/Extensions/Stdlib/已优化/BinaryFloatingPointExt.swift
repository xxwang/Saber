...
public extension SaberExt where Base: BinaryFloatingPoint {
    ...
    /// 四舍五入转Int
    var lround: Int {
        return Darwin.lround(Double(self.base))
    }
...
public extension SaberExt where Base: BinaryFloatingPoint {
    ...
    func truncate(places: Int) -> Base {
        let divisor = pow(10.0, places.sb.double)
        return Base(double * divisor / divisor)
    }
    ...
    func round(_ places: Int) -> Base {
        let divisor = pow(10.0, places.sb.double)
        return Base((double * divisor).rounded() / divisor)
    }
    ...
    func rounded(_ places: Int, rule: FloatingPointRoundingRule) -> Base {
        ...
        return (self.base * factor).rounded(rule) / factor
    }
