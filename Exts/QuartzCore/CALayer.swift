...
// MARK: - 方法
public extension SaberExt where Base: CALayer {
    /// 图层转图片(需要图层已经有`size`)
    /// - Returns: `UIImage?`
    func toImage() -> UIImage? {
        UIGraphicsBeginImageContext(base.frame.size)
...
public extension SaberExt where Base: CALayer {
    ...
    func baseBasicAnimation(
        ...
        // 添加动画到图层
        base.add(basicAnimation, forKey: nil)
    }
